...
for i=1:lenSampleData
    ...
    if iVar > 0 && size(sample_data{iSort(i)}.variables{iVar}.data, 2) == 1 % we're only plotting 1D variables
        if initiateFigure
        ...
        if all(~iGood)
            ...
            hLineVar(i + 1) = line(sample_data{iSort(i)}.dimensions{iTime}.data(iGood), ...
