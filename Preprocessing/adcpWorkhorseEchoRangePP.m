function [sample_data] = adcpWorkhorseEchoRangePP( sample_data, auto )
...
if strcmpi(sample_data.dimensions{idVertDim}.name, 'DIST_ALONG_BEAMS')
    disp(['Warning : adcpWorkhorseEchoRangePP applied with a non tilt-corrected ABSICn (no bin mapping) on dataset ' sample_data.toolbox_input_file]);
end
...
try
    ...
    try
        is_beam = unique(sample_data.meta.fixedLeader.coordinateTransform) ~=7;
    catch
        is_beam = false;
    end
end
...
sample_data.variables{idVel1}.flags = squeeze(flags(1,:,:));
sample_data.variables{idVel2}.flags = squeeze(flags(2,:,:));
sample_data.variables{idVel3}.flags = squeeze(flags(3,:,:));
sample_data.variables{idVel4}.flags = squeeze(flags(4,:,:));
